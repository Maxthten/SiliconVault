...
const bundlePath = path.join(__dirname, '.quality-integration.cjs')
const singletonRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'siliconvault-quality-singleton-'))

try {
  esbuild.buildSync({
    entryPoints: [path.join(__dirname, 'quality-integration.ts')],
    outfile: bundlePath,
    bundle: true,
    platform: 'node',
    format: 'cjs',
